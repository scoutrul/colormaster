...
export function generateSystem(state: AppState): DesignTokens {
  ...
  const bgSecondary = { 
    l: isDark ? Math.min(bgL + stepL + intensityShift, 0.16) : Math.max(bgL - stepL - intensityShift, 0.90), 
    c: isDark ? bgC + 0.01 : bgC + 0.005, 
    h: targetHue 
  };
  
  const bgTertiary = {
    l: isDark ? Math.min(bgSecondary.l + stepL, 0.22) : Math.max(bgSecondary.l - stepL, 0.82),
  ...
  return {
    ...
    typography: {
      ...
      serifFamily: typography.editorialFont,
      displayFamily: typography.displayFont,
    }
  };
}
